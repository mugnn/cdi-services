...
const groups = ["N3-MI-OUTSOURCING", "N3-IVA", "N3-MI-OUTSOURCING-24X7", "N3-SERVIDORES_OUTSOURCING_CFTV"]
...
async function updatePendingIncidents() {
  ...
  for (const group of groups) {
    await axios.get(`https://api.petrobras.com.br/ti/itsm/v1/itsm-chamados/chamados?mesa_atendimento=${group}&tipo_servico=incident&status=Pending`, auth)
      .then(response => {
...
async function updateActiveIncidents() {
  ...
  for (const group of groups) {
    await axios.get(`https://api.petrobras.com.br/ti/itsm/v1/itsm-chamados/chamados?mesa_atendimento=${group}&tipo_servico=incident&status=Open`, auth)
      .then(response => {
  ...
  return { new: newCall, progress: progress };
}
